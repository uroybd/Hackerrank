...
(defn parser [coll]
  (if (seq? coll)
    (map #(cond (= (.indexOf coll %) 0) (symbol %)
                ...
                :else (let [x %]
                        `(deref ~(symbol (str "whl-" x)))))
         coll)
    (if (re-find #"\d+" coll)
      (Integer/parseInt coll)
      `(deref ~(symbol (str "whl-" coll))))))
...
(definer "val := 10000 ;")
(definer "cur := val ;")
(definer "mod := 1000000007 ;")

(while-builder "while ( cur > 1 )" ["fact := fact * cur ;" "fact := fact - fact / mod * mod ;" "cur := cur - 1"])

(clojure.core/loop []
  (clojure.core/when (> (clojure.core/deref whl-cur) 1)
    (do (def whl-fact (clojure.core/atom (* (clojure.core/deref whl-fact) (clojure.core/deref whl-cur))))
        (def whl-fact (clojure.core/atom (- (clojure.core/deref whl-fact) (* (/ (clojure.core/deref whl-fact) (clojure.core/deref whl-mod)) (clojure.core/deref whl-mod)))))
        (def whl-cur (clojure.core/atom (- (clojure.core/deref whl-cur) 1)))) (recur)))

(loop []
  (when (> cur 1)
    (do (def fact (atom (* @fact @cur)))
        (def fact (atom (- @fact (* (/ @fact @modi) @modi))))
        (def cur (atom (- cur 1))))
    (recur)))
